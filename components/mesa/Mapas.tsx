...
interface MapasProps {
  ...
  // Novas props para Zoom e Redimensionamento do Mapa
  mapScale?: number;
    ...
    <div className="space-y-4">
      ...
      {activeMapUrl && (
        <div className="p-3 bg-[#0b0c16] border border-cyan-800/40 rounded-xl space-y-3">
          <span className="block text-xs font-bold text-cyan-300 uppercase">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-full accent-cyan-400 cursor-pointer h-1.5 bg-[#12131f] rounded-lg"
                />
                <button
                  type="button"
                  onClick={() => onMapScaleChange(100)}
                  className="px-2 py-0.5 bg-purple-900/60 hover:bg-purple-800 text-[9px] font-bold text-purple-200 rounded transition cursor-pointer"
      ...
      <div className="space-y-2">
        ...
        {maps.length === 0 ? (
          <p className="text-xs text-gray-500 text-center py-4">Nenhum mapa carregado.</p>
        ) : (
          maps.map((map) => (
            <div
              key={map.id}
              className={`p-2 bg-[#0b0c16] rounded-xl border flex items-center justify-between gap-2 ${
                map.image_url === activeMapUrl ? "border-cyan-400 glow-blue" : "border-purple-900/40"
              }`}
            >
              <img
                src={map.image_url}
              ...
              />
              <span className="text-xs text-white font-medium flex-1 truncate">{map.name}</span>

              {isMestre && (
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => onSelectMap(map)}
                    className="px-2.5 py-1 bg-purple-900/60 hover:bg-cyan-600 text-[10px] font-bold text-white rounded-lg transition cursor-pointer"
                  >
                    Ativar
                  </button>
